...
public class BillConvertUtils {
     ...
     * @return 转换后的 Bill 实体
     * @throws BillBusinessException 转换失败时抛出业务异常
     */
    public static Bill mapToBill(Map<String, Object> billMap) {
        // 1. 校验输入Map
        if (billMap == null || billMap.isEmpty()) {
            throw new BillBusinessException("map to bill failed：输入Map为空，无法转换");
        }
        ...
        try {
            ...
            if (id == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "账单ID"));
            }
            bill.setId(id);
            ...
            if (billTime == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "账单时间"));
            }
            ...
            if (type == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "收支类型"));
            }
            // 校验收支类型合法性
            if (type != -1 && type != 1) {
                throw new BillBusinessException("收支类型值非法：" + type + "，仅支持-1（支出）或1（收入）");
            }
            bill.setType(type);
            ...
            if (categoryId == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "大类ID"));
            }
            ...
            if (specificTypeId == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "具体类型ID"));
            }
            ...
            if (amount == null) {
                throw new BillBusinessException(String.format(NULL_FIELD_MSG, "金额"));
            }
            // 校验金额大于0
            if (amount.compareTo(BigDecimal.ZERO) <= 0) {
                throw new BillBusinessException("金额非法：" + amount + "，需大于0");
            }
            ...
            throw new BillBusinessException("map to bill failed：转换异常，原因：" + e.getMessage());
        }
    ...
    private static Integer getIntegerValue(Map<String, Object> map, String key, String fieldName) {
        ...
        if (value instanceof String) {
            ...
            try {
                ...
                throw new BillBusinessException(fieldName + "格式错误：" + value + "，需为整数");
            }
        }
        // 简化类型名称，提升异常可读性
        String actualType = value.getClass().getSimpleName();
        throw new BillBusinessException(fieldName + "类型错误：" + actualType + "，不支持转换为Integer");
    }
    ...
    private static BigDecimal getBigDecimalValue(Map<String, Object> map, String key, String fieldName) {
        ...
        if (value instanceof String) {
            ...
            try {
                ...
                throw new BillBusinessException(fieldName + "格式错误：" + value + "，需为数字（支持整数/小数）");
            }
        }
        // 兼容所有数字类型，避免浮点精度丢失
        if (value instanceof Number) {
        ...
        // 简化类型名称，提升异常可读性
        String actualType = value.getClass().getSimpleName();
        throw new BillBusinessException(fieldName + "类型错误：" + actualType + "，不支持转换为BigDecimal");
    }
    ...
    private static String getTimeStringValue(Map<String, Object> map, String key, String fieldName) {
        ...
        if (value instanceof String) {
            ...
            // 其他类型不支持
            String actualType = value.getClass().getSimpleName();
            throw new BillBusinessException(fieldName + "类型错误：" + actualType + "，不支持转换为时间字符串");
        }
        ...
        try {
            ...
            throw new BillBusinessException(fieldName + "格式错误：" + timeStr + "，需符合格式：" + example);
        }
